...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // After upload + backend pipeline → go to metrics
      router.push("/metrics");
    } catch (err) {
      alert("Upload failed. Please check log format.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div>
          ...
          <div className="mt-10 border border-slate-200 rounded-2xl p-10 bg-white shadow-sm">
            ...
            <div className="mt-6 text-sm text-slate-500 space-y-2">
              ...
              <p>• Structured & unstructured logs supported</p>
              <p>• Includes timestamp, level, endpoint, message (recommended)</p>
            </div>
